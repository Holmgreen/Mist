...
public abstract class WorldState extends GameState {
	...
	if (ooru.getCurrentRow() == starty[EAST] / map.getTileSize() && ooru.getCurrentCol() == startx[EAST] / map.getTileSize()) {
	    gsm.setState(exitState[EAST]);
	}
    }

    protected void checkWestExit() {
	if (ooru.getCurrentRow() == starty[WEST] / map.getTileSize() && ooru.getCurrentCol() == startx[WEST] / map.getTileSize()) {
	...
	ui.update();
	map.update();
	map.setPosition(GamePanel.WIDTH / 2 - ooru.getx(), GamePanel.HEIGHT / 2 - ooru.gety());
    }
