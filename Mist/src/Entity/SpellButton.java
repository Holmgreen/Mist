...
public class SpellButton extends Rectangle {
	...
	try {
	    buttonImage = ImageIO.read(getClass().getResourceAsStream("/Objects/button.png"));
	    buttonCdImage = ImageIO.read(getClass().getResourceAsStream("/Objects/buttoncd.png"));
	} catch (Exception e) {
	    e.printStackTrace();
	}
    }
	...
	if (spell != null) {
		...
		g.setFont(cdFont);
		g.drawString("" + spell.getTimeRemaining() / 1000, x + 2, y + 10);
	    } else {
		g.drawImage(buttonImage, x, y, width, height, null);
	    }
		...
		g.fillRect(x, y - spell.getStringRows()*(g.getFontMetrics().getHeight()) - height, spell.getLength()*4 + 3, spell.getStringRows()*(g.getFontMetrics().getHeight()) + 9);
		g.setColor(infoColor);
		g.setFont(infoFont);
		drawString(g, spell.getString(), x + 1, y - spell.getStringRows()*(g.getFontMetrics().getHeight()) - height);
	    }
	}
    }
