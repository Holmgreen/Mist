...
public class TellingState extends GameState {
	...
	story[0] = "Once upon a time, in the primitive lands of \nAlteriam, the young one lived. \nWith a power yet to be discovered.";
	story[1] = "He was not aware of his fate nor was he in\ncharge of it.";
	story[2] = "One day, the rapidly growing evil would no \nlonger be transparent in the eyes of the \nyoung one.";
...
//		JukeBox.play("typing");
	...
	if (Keys.isPressed(Keys.ENTER)) end = (story[index].length());
	if (Keys.isPressed(Keys.ESCAPE)) gsm.setState(GameStateManager.HOMEHOUSESTATE);
    }
